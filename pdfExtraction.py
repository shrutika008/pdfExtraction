    ...
    with st.sidebar:
        ...
        if st.button("Submit & Process"):
            with st.spinner("Processing..."):
                ...
                print(raw_text[:2000])  # Print the first 2000 characters of the raw text for debugging
                text_chunks = get_text_chunks(raw_text)
                print("Text chunks:")
                for i, chunk in enumerate(text_chunks[:5]):  # Print the first 5 chunks for debugging
                    print(f"Chunk {i+1}:")
                    print(chunk)
                    print("\n---\n")
